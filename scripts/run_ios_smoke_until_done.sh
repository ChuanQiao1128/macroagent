#!/usr/bin/env bash
# Run the v0.5 iOS native capture smoke-test queue until selected tasks finish.
#
# Default queue:
#   TASK-043 TASK-044 TASK-045 TASK-046 TASK-047
#
# Usage:
#   CODEX_PROVIDER_MODE=chatgpt MAX_REPAIR_ATTEMPTS=3 \
#     bash scripts/run_ios_smoke_until_done.sh
#
#   bash scripts/run_ios_smoke_until_done.sh TASK-043 TASK-044
#
#   V05_DRY_RUN=1 bash scripts/run_ios_smoke_until_done.sh
...
DEFAULT_TASKS=(TASK-043 TASK-044 TASK-045 TASK-046 TASK-047)
TASKS=("$@")
if [[ "${#TASKS[@]}" -eq 0 ]]; then
  TASKS=("${DEFAULT_TASKS[@]}")
fi

STATE_FILE="${V05_STATE_FILE:-$ROOT/.codex/runs/ios_smoke_v05_state.tsv}"
MAIN_BRANCH="${MAIN_BRANCH:-v05-ios-smoke-main}"
...
record_task_state() {
  ...
  tmp_file="$(mktemp "${TMPDIR:-/tmp}/macroagent-v05-state.XXXXXX")"
...
echo "MacroAgent v0.5 iOS native capture smoke queue"
echo "Tasks: ${TASKS[*]}"
...
echo "MacroAgent v0.5 iOS native capture smoke queue complete."
